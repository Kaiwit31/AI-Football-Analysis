...
class Tracker:
    ...
    def detect_frames(self, frames):
        bacth_sizes=20
        detections = []
        for i in range(0,len(frames),bacth_sizes):
            detections_batch = self.model.predict(frames[i:i+bacth_sizes],conf=0.1)
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        for frame_num, detection in enumerate(detections):
            ...
            #Covert to Supervision Detection formet
    ...
    def draw_triangle(self,frame,bbox,color):
        ...
        traiangle_points = np.array([
            [x,y],
            [x-10,y-20],
            [x+10,y-20],
        ])
        cv2.drawContours(frame,[traiangle_points], 0,color, cv2.FILLED)
        cv2.drawContours(frame,[traiangle_points], 0,(0,0,0), 2)
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        # Get the number of time each team had a ball control
